...
.. activecode:: ds_05_countries_with_more
    ...
    Suppose the dictionary ``golds`` maps countries to the number of gold medals they won in the 2016 olympics. Write code to store the names of countries that won more than 20 gold medals into the variable ``gold20``.
    ~~~~
...
.. activecode:: ds_05_state_biggest
   :language: python
   :autograde: unittest
   :chatcodes:
   :practice: T
   ...
   We have ``states.txt`` from before. This time, write code to find the state with the **largest** population and store the name of the state (e.g., ``"Michigan"``) in the variable ``largest_pop``. Do not hard code your answer.
   ~~~~
   f = open('states.txt', 'r')
   for line in f.readlines():
       cells = line.split(',')
   ...
   class myTests(TestCaseGui):
       def testOne(self):
           self.assertEqual(largest_pop, "California", "Testing largest_pop")
           self.assertIn('>', self.getEditorText(), "Testing your code (Don't worry about actual and expected values).")

   myTests().main()

.. activecode:: ds_05_writing_files
    :language: python
    :autograde: unittest
    :practice: T
    :available_files: nums.txt

    Write code that writes the string ``"hello world"`` to a file named ``myfile.txt``.
    ~~~~
    # your code
    =====
    ...
    class myTests(TestCaseGui):
        ...
        def testOneA(self):
            ...
            self.assertEqual(d, "hello world", "Testing that myfile.txt has the correct contents.")
