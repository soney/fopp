...
.. activecode:: lec_02_01
    :language: python
    :autograde: unittest

    **1.** The code below should print out ``hello 106`` but it produces a syntax error. Fix the code so that it runs correctly **and add a comment explaining why the previous code did not work**.
    ~~~~
    print("hello 106')
    ...
    class myTests(TestCaseGui):
        ...
        def testOne(self):
           self.assertIn('hello 106', self.getOutput(), "Testing that you printed hello 106")
...
.. activecode:: ps_01_02
    ...
    **2.** The code below should print out ``10`` but it produces a syntax error. Fix the code *by removing spaces* so that it runs correctly **and add a comment explaining why the previous code did not work**.
    ~~~~
       print( 5 + 5 )
    ...
    class myTests(TestCaseGui):
        ...
        def testOne(self):
           self.assertIn('10', self.getOutput(), "Testing that you printed 10")
...
.. activecode:: ps_01_03
    ...
    **3.** The code below should print out the value of ``x+y`` (``15``) but it does not. Fix the code so that it runs correctly **and add a comment explaining why the previous code did not work**.
    ~~~~
    x = 5
    y = 10
    print('x + y')
    ...
    class myTests(TestCaseGui):
        ...
        def testOne(self):
           self.assertIn('15', self.getOutput(), "Testing that you printed 15")
...
.. activecode:: ps_01_04
    ...
    **4.** The code below should print out ``v1 + 2v`` but it produces a syntax error. Fix the code *by renaming a variable* so that it runs correctly **and add a comment explaining why the previous code did not work**.
    ~~~~
    v1 = "VA"
    2v = "LUE"
    print(v1 + 2v)
    ...
    class myTests(TestCaseGui):
        ...
        def testOne(self):
           self.assertIn('VALUE', self.getOutput(), "Testing that you printed VALUE")
...
.. activecode:: ps_01_05
    ...
    **6.** Write code to assign the number of *characters* in the string ``rv`` to a variable ``num_chars``. Then write code to assign the number of *words* in the string ``rv`` to the variable ``num_words``. (Hint: remember how to split strings?)
    ~~~~
...
.. activecode:: ps_01_06
    ...
    **6.** There is a function we are providing in for you in this problem set called ``square``. It takes one integer and returns the square of that integer value. Write code to assign a variable called ``xyz`` the value ``5*5`` (five squared). Use the square function, rather than just multiplying with ``*``.
    ~~~~
    xyz = "re-assign me!"
...
.. activecode:: ps_01_07
    ...
    **7.** Write in a comment next to each line of code, saying what each line does. (You should be very specific! This exercise will train your brain for when you write more complicated code.)
    ~~~~
    course_A = 'SI 106, 4'
    course_B = 'SI 110, 4'
...
.. activecode:: ps_01_11
    ...
    **11.** There is a function we have defined that takes two strings as inputs, and returns the length of both of those strings added together. It's called called ``add_lengths``. We are also including the functions from Problem Set 1 called ``random_digit`` and ``square`` in this problem set.
...
.. activecode:: ps_01_12
    ...
    **12.** Assign the value of the third element of ``num_lst`` to a variable called ``third_elem``.
...
.. activecode:: ps_01_13
    ...
    **13.** Write a program that uses the turtle module to draw something. It doesn't have to be complicated, but draw something different than we did in the textbook or in class. (Optional but encouraged: post a screenshot of the artistic outcome to the discussion board, or a short video of the drawing as it is created.) (Hint: if you are drawing something complicated, it could get tedious to watch it draw over and over. Try setting ``.speed(10)`` for the turtle to draw fast, or ``.speed(0)`` for it to draw super fast with no animation.)
    ~~~~
    import turtle
